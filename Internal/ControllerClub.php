...
if(isset($_POST['Clubupdate']))
{
	...
	// Process lic upload (consider validation for PDF format)
    ...
    if($isValid)
    {
    	$q = "UPDATE clubs SET location='$location',founded_year=$founded_year,owner_name='$owner_name',owner_phono='$ph_no',website='$website',lic_no='$lic_no',lic_doc='$target_file2' WHERE email='$Email'"; echo $q;
...
if(isset($_GET['Action']))
{
   ...
   // $FieldName = "status";
    ...
    $Query_reg = "INSERT INTO $TableName(club_id,pid,role,status) VALUES($club_id,$pid,'$role','New')"; echo $Query_reg;
    $Result_reg = mysql_query($Query_reg); 
    $Query_log = "UPDATE player_reg SET acquired_status='Acquired' WHERE pid = '$pid'"; echo $Query_log;
...
if(isset($_GET['Action2']))
{
   ...
   // $FieldName = "status";
    $Unique_Id = $_GET['id']; // Unique ID like Email
    $Status = $_GET['Action2']; // Approve OR Reject will come
    $email = $_SESSION['username'];

    $Qfetch = "SELECT pid FROM player_matchinfo WHERE id='$Unique_Id'";
    $Result= mysql_query($Qfetch); 
    $Rowf = mysql_fetch_assoc($Result);

    $pid = $Rowf['pid'];

    $Query_reg = "UPDATE $TableName SET status='Approved',club_id='$email' WHERE pid = '$pid'"; echo $Query_reg;
    $Result_reg = mysql_query($Query_reg); 
...
if($_GET['Action3'] == 1)
{
	...
	// $Q0 ="SELECT * FROM suggested_players WHERE pid=$pid";
	// $Result = mysql_query($Q0);
